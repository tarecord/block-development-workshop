...
function bdw_workshop_block_init() {

    // automatically load dependencies and version
    $asset_file = include( plugin_dir_path( __FILE__ ) . 'build/index.asset.php');

    wp_register_script(
        'bdw-workshop-block-script',
        plugins_url( 'build/index.js', __FILE__ ),
        $asset_file['dependencies'],
        $asset_file['version']
    );

    register_block_type( 'bdw-workshop/workshop-block', array(
        'apiVersion' => 2,
        'editor_script' => 'bdw-workshop-block-script',
    ) );

}
add_action( 'init', 'bdw_workshop_block_init' );
